...
exports.protocolParser = exports.ProtocolParser = void 0;
...
class ProtocolParser {
     ...
     */
    parse(order) {
        const protocolName = order.medicationCodeableConcept?.text ||
            order.medicationCodeableConcept?.coding?.[0]?.display ||
            "Chemotherapy Regimen";
        const rxNormCode = order.medicationCodeableConcept?.coding?.find((c) => c.system.includes("rxnorm") || c.system === "http://hl7.org/fhir/sid/rxnorm")?.code;
        ...
        if (activeIngredients.length === 0 && rxNormCode) {
            const known = rxnorm_js_1.COMMON_RXNORM_CATALOG[rxNormCode];
        ...
        if (order.dosageInstruction && order.dosageInstruction.length > 0) {
            order.dosageInstruction.forEach((instr, idx) => {
                ...
                let rateQuantity = instr.doseAndRate?.[0]?.rateQuantity
                    ...
                    : undefined;
                const durationHours = instr.timing?.repeat?.duration && instr.timing.repeat.durationUnit === "h"
                    ? instr.timing.repeat.duration
                    : undefined;
                // Calculate rate if rateQuantity not explicitly set, but total volume and duration are present
                let calculatedRateMlPerHour = rateQuantity?.value;
                if (!calculatedRateMlPerHour && durationHours && carrierFluids.length > 0) {
                    const totalVol = carrierFluids.reduce((acc, cf) => acc + (cf.strengthRatio?.numeratorValue || 0), 0);
                    if (totalVol > 0) {
        ...
        const requiresCentralLine = protocolName.toLowerCase().includes("folfox") ||
    ...
    extractIngredientsFromMedication(med, activeIngredients, carrierFluids) {
        if (!med.ingredient || med.ingredient.length === 0) {
            ...
            const known = rxnorm_js_1.COMMON_RXNORM_CATALOG[code];
            activeIngredients.push({
        ...
        for (const ing of med.ingredient) {
            ...
            const strengthRatio = ing.strength
                ...
                : undefined;
            const isCarrier = known?.category === "CARRIER_FLUID" ||
                ing.isActive === false ||
...
exports.protocolParser = new ProtocolParser();
