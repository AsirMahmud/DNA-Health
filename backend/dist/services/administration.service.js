...
class AdministrationService {
    ...
    updateSafetyGateStatus(patientId, assessment) {
     ...
     */
    ingestHl7RasMessage(rawHl7) {
        const event = rasO17Handler_js_1.rasO17Handler.parseRasMessage(rawHl7);
        // Match patient by MRN or ID
        const patients = client_js_1.fhirClient.getAllPatients();
        const matchedPatient = patients.find((p) => p.identifier?.some((id) => id.value === event.patientMrn));
        ...
        if (gateInfo && !gateInfo.allowed && event.completionStatus !== "COMPLETED") {
            ...
            audit_js_1.auditEventService.logBedsideAdministrationEvent({
                patientId,
        ...
        const fhirAdmin = rasO17Handler_js_1.rasO17Handler.toFhirMedicationAdministration(event, patientId);
        client_js_1.fhirClient.addMedicationAdministration(patientId, fhirAdmin);
        // Audit log
        audit_js_1.auditEventService.logBedsideAdministrationEvent({
            patientId,
