...
exports.hl7Parser = exports.Hl7Parser = void 0;
class Hl7Parser {
    /**
    ...
    parse(rawMessage) {
        ...
        const segments = rawSegments.map((segLine) => this.parseSegment(segLine, encoding));
    ...
    parseField(fieldStr, encoding) {
        ...
        const repetitions = fieldStr.split(encoding.repetitionSeparator).map((rep) => {
            return rep.split(encoding.componentSeparator).map((comp) => {
                const subcomps = comp.split(encoding.subcomponentSeparator).map((s) => this.unescape(s, encoding));
                return {
                    value: subcomps[0] || "",
     ...
     */
    getFieldComponentValue(segment, fieldIndex, componentIndex = 1, encoding) {
        const field = segment.fields[fieldIndex - 1];
        if (!field || field.repetitions.length === 0)
            return "";
...
exports.hl7Parser = new Hl7Parser();
