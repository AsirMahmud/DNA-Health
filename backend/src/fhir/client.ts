...
export interface ClinicalPatientContext {
  ...
  auditEvents: FhirAuditEvent[];
}

export class FhirClient {
  private patientData: Map<string, ClinicalPatientContext> = new Map();

  constructor() {
    this.seedClinicalCohorts();
  }

  private seedClinicalCohorts(): void {
    const now = new Date();
    ...
    const folfoxMedRequest: FhirMedicationRequest = {
    ...
    const p1001Labs: FhirObservation[] = [
      {
      ...
      },
      {
        ...
        code: {
          coding: [
            { system: "http://loinc.org", code: LOINC_CODES.PLATELETS, display: "Platelets [#/volume] in Blood" },
          ],
          text: "Platelet Count",
        },
        subject: { reference: "Patient/P-1001" },
    ...
    const rchopMedRequest: FhirMedicationRequest = {
      resourceType: "MedicationRequest",
      id: "MR-RCHOP-1002",
      status: "active",
      intent: "order",
    ...
    const p1002Labs: FhirObservation[] = [
      {
        resourceType: "Observation",
        id: "obs-1002-anc",
        status: "final",
        code: {
          coding: [{ system: "http://loinc.org", code: LOINC_CODES.ANC_AUTOMATED, display: "Neutrophils [#/volume] in Blood" }],
          text: "Absolute Neutrophil Count (ANC)",
        },
      ...
      },
      {
        ...
        status: "final",
        code: {
          coding: [{ system: "http://loinc.org", code: LOINC_CODES.CREATININE_SERUM_OR_PLASMA, display: "Creatinine Serum" }],
          text: "Serum Creatinine",
        },
        subject: { reference: "Patient/P-1002" },
        effectiveDateTime: twoHoursAgo,
    ...
    const cisplatinMedRequest: FhirMedicationRequest = {
      resourceType: "MedicationRequest",
      id: "MR-CIS-1003",
      status: "active",
      intent: "order",
    ...
    const p1003Labs: FhirObservation[] = [
      {
        resourceType: "Observation",
        id: "obs-1003-anc",
        status: "final",
        code: {
          coding: [{ system: "http://loinc.org", code: LOINC_CODES.ANC_AUTOMATED, display: "Neutrophils in Blood" }],
    ...
    const p1004MedRequest: FhirMedicationRequest = {
      resourceType: "MedicationRequest",
      id: "MR-FOLFOX-1004",
      status: "active",
      intent: "order",
    ...
    const p1004Labs: FhirObservation[] = [
      {
        resourceType: "Observation",
        id: "obs-1004-anc",
        status: "final",
  ...
  public getAllPatients(): FhirPatient[] {
    return Array.from(this.patientData.values()).map((ctx) => ctx.patient);
  }

  public getPatient(patientId: string): FhirPatient | null {
    return this.patientData.get(patientId)?.patient || null;
  }

  public getMedicationRequests(patientId: string): FhirMedicationRequest[] {
    return this.patientData.get(patientId)?.medicationRequests || [];
  }

  public getObservations(patientId: string): FhirObservation[] {
    return this.patientData.get(patientId)?.observations || [];
  }

  public getAdministrations(patientId: string): FhirMedicationAdministration[] {
    return this.patientData.get(patientId)?.administrations || [];
  }

  public getAuditEvents(patientId: string): FhirAuditEvent[] {
    return this.patientData.get(patientId)?.auditEvents || [];
  }

  public addMedicationAdministration(patientId: string, admin: FhirMedicationAdministration): void {
  ...
  public addAuditEvent(patientId: string, audit: FhirAuditEvent): void {
    const ctx = this.patientData.get(patientId);
    if (ctx) {
